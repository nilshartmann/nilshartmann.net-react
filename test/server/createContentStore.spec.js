...
describe("createContentStore", () => {
  ...
  describe("createWritableContentStore", () => {
    ...
    beforeEach(cb => {
      testEnv = initTestEnv();
      testEnv
        .initGitRepositoryFrom(TESTCONTENT_DIR)
        .then(repoDir => {
    ...
    it("publishes a post", () => {
      return contentStore.publishPost("sonnenuntergang").then(() => {
    ...
    it('removes all "protected" properties from post', () => {
      ...
      return contentStore.storeDraft(postWithHiddenProperties).then(() => {
        expect(testEnv.readJson("drafts/post-with-hidden-properties.json"), "to equal", expectedStoredPost);
      });
    });
    ...
    it("list uploaded files ordered by date", () => {
      const expectedFiles = [{ file: "uploads/logo_small.png" }, { file: "uploads/eclipse.png" }];
      return contentStore
        .listUploadedFiles()
        .then(files => {
          console.log(files);
