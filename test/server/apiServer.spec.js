...
describe("API Server", () => {
  ...
  describe("admin API", () => {
    let loginResult;
    beforeEach(() => {
      return _login().then(res => (loginResult = res));
    });
    ...
    it("should NOT have access to restricted route without token", () => {
      return server.inject("/api/restricted").then(res => {
        expect(res.statusCode, "to equal", 401);
      });
    });
  ...
  describe("public read API", () => {
    ...
    it("should find a post", () => {
      const p = server.inject("/api/posts/osgi-by-example").then(res => res.result);
      return expect(p, "when fulfilled", "to satisfy", { slug: "osgi-by-example" });
    });

    it("should return all tags", () => {
      const p = server.inject("/api/tags").then(res => res.result);
