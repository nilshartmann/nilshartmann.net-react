...
export default function createModel({ initialState, backendUrl, history = DUMMY_HISTORY, localStorage = DUMMY_LOCAL_STORAGE }) {
  ...
  const actions = Object.assign(
    {},
    {
        ...
        return fetch(url, params)
          .then(response => {
            if (response.ok) {
              ...
              mergeState({ authorization }, () => actions.pushLocation(redirectOnSuccess));
            } else {
      ...
      logout() {
        return doFetch(`${backendUrl}/api/logout`).then(discardAuthorization).then(actions.pushLocation("/")).catch(ex => {
      ...
      loadPost(slug) {
        ...
        return doFetch(`${backendUrl}/api/posts/${slug}`).then(handleResponse).catch(ex => console.error("parsing failed", ex));
      },
      ...
      getFlickrImages(count = 6) {
        ...
        return getOrFetchFlickrImages(`${backendUrl}/api/flickrimages`).then(allFlickrImages => {
      ...
      savePost(post) {
        return actions.postJson(`/api/posts`, { post });
      },
      ...
      listUploads() {
        ...
        return fetch(url, params).then(res => res.json()).then(uploads => mergeState({ uploads }));
      }
    }
  );
