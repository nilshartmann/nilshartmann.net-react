...
function PostTeaser({post, pushLocation}) {
  ...
  return <div onClick={ () => pushLocation(postUrl) } className='Row PostTeaser Selectable'>
    <div className='TitlePanel Clearfix'>
      {image}
      {post._date}
      <h1 className='Title'>{post.title}</h1>
    </div>
    <p><span dangerouslySetInnerHTML={{ __html: post._summaryHtml }}/>&nbsp;<Link className='ReadMore' to={postUrl}>More...</Link>
    </p>
  </div>;
}
