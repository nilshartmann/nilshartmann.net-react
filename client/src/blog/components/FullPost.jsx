...
export default class FullPost extends React.Component {
  ...
  renderPostImage(post) {
    ...
    if (this.state.imageExpanded) {
      return <div className='ImageFullWidth-Full'><img className='ImageFullWidth-Full' src={`${post.image}`}
                                                       onClick={this.toggleExpandedImage}/></div>;
    }
  ...
  render() {
    ...
    return <div className='Row FullPost'>
      ...
      <div>
        ...
        <div className='Summary' dangerouslySetInnerHTML={{ __html: post._summaryHtml }}></div>
        <div className='Content' dangerouslySetInnerHTML={{ __html: post._contentHtml }}></div>
      </div>
      <div className='Tags'>
        {post.tags.map((tag) => {
          return <Button key={tag} linkTo={tagUrl(tag)}>{tag}</Button>;
        })}
      </div>
