...
export default class PostEditor extends React.Component {
  ...
  save() {
    ...
    savePost(newPost).then( ({response}) => {
  ...
  render() {
    ...
    <div className='Column-2'>
      ...
      <div className='Row PostEditor'>
        ...
        <TextInputField label='Slug' value={post.slug} onValueChange={this.onFieldChange}
                        onRefresh={this.refreshSlug} errorMessage={errors.slug} enabled={post._new === true} />
        <TextInputField label='Date' name='_date' value={post._date} onValueChange={this.onFieldChange}/>
        <TextInputField label='Image (URL)' name='image' value={post.image}
        ...
        <div className='EditorGroup'>
          <input type='checkbox'
                 ...
                 className='Control'>
          </input>Show image expanded
        </div>


        <TextInputField label='Summary (Markdown)' name='summary' rows={3} value={post.summary}
                        onValueChange={this.onFieldChange}/>
        <TextInputField label='Content (Markdown)' name='content' rows={10} value={post.content}
                        onValueChange={this.onFieldChange}/>
        ...
        {post.tags.map((tag) => {
          ...
          return <div className='Tag' key={tag}>{label}<i className='fa fa-times'
                                                        onClick={() => { this.onTagRemove(tag); }}/></div>;
        })}
