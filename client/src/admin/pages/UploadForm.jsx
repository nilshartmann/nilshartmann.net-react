...
class UploadForm extends React.Component {
  ...
  onUploadFile() {
    ...
    postForm("/api/upload", data).then(() => listUploads());
  }
        ...
        {uploads.map(upload =>
          <div key={upload.file} style={{ marginBottom: "30px", textAlign: "center" }}>
            <a href={`/${upload.file}`} target="_blank">
              <img src={`/${upload.file}`} style={{ width: "200px", margin: "0 auto", display: "block" }} />
            </a>
            /{upload.file}
            <button onClick={() => copytext("/" + upload.file)}>Copy</button>
          </div>
        )}
      </div>
    );
...
export default connectModel(
  UploadForm,
  ({ uploads }) => ({ uploads }),
  ({ postForm, listUploads }) => ({ postForm, listUploads })
);
