...
export default function renderRoute(request, reply, backendUrl) {
  ...
  match({ routes, location }, (error, redirectLocation, renderProps) => {
    if (redirectLocation) {
      ...
      preRender(renderProps.components, renderProps, model)
        .then(() => {
...
function preRender(components, renderProps, model) {
  ...
  const preRenderPromises = preRenderComponents.map(component => component.preRender(renderProps, model.actions));
