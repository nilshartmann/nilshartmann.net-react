...
export function configureReadApiRoutes(server, { publicApi, authenticatedApi }) {
  ...
  server.route({
    ...
    handler: (request, reply) => {
      _api(request)
        .queryPosts(request.query.options)
        .then(posts => {
  ...
  server.route({
    ...
    handler: (request, reply) => {
      _api(request)
        .getTags(request.query.count === true)
        .then(tags => {
  ...
  server.route({
    ...
    handler: (request, reply) => {
      _api(request).getFlickrImages().then(photos => reply(photos).type("application/json")).catch(err => reply(Boom.wrap(err)));
    }
  });
}
