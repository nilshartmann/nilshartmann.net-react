...
export function configureWriteApiRoutes(server, api, jwtKey) {
  ...
  server.route({
    ...
    handler: (request, reply) => {
      ...
      reply({ text: "Login successful" }).header("Authorization", token);
    },
    config: {
      payload: {
        parse: true
  ...
  server.route({
    ...
    handler: (request, reply) => {
      ...
      api.publishPost(slug).then(() => reply({ msg: "ok" })).catch(err => {
  ...
  server.route({
    ...
    path: "/api/uploads",
    handler: (request, reply) => {
      api.listUploads().then(files => reply(files));
  ...
  server.route({
    ...
    handler: (request, reply) => {
      ...
      api
        .upload(payload, formFile.hapi.filename, formFile)
        .then(uploadedFile => {
